...
export default {
	...
	beforeMount() {
		const activeRoutes = this.getPathUntilCurrentRoute(
			this.routesMethod(),
			this.$route,
		);
		let subMenu = [];
	...
	computed: {
		routes() {
			return this.filterSidebarRoutes(routes).map((route) =>
				this.prepareForMenu(route),
			);
		},
		loading() {
	...
	methods: {
		filterSidebarRoutes(routes) {
			return routes.filter((route) => {
				if (route.meta.sidebar && route.children) {
					route.sidebarChildren = this.filterSidebarRoutes(
						route.children,
					);
				}
		...
		getPathUntilCurrentRoute(routes, current) {
			for (let i = 0; i < routes.length; i++) {
				...
				if (route.children && route.children.length > 0) {
					let routesTree = this.getPathUntilCurrentRoute(
						route.children,
						current,
					);
		...
		prepareForMenu(route) {
			...
			if (route.sidebarChildren) {
				result.children = route.sidebarChildren.map((item) =>
					this.prepareForMenu(item),
				);
