<template>
	...
	>
		<a-space class="site-input-group-wrapper w-full" direction="vertical">
			...
			<div
				class="max-h-[540px] flex items-center flex-col p-4 overflow-y-auto border-2 rounded"
			>
				<div
					v-for="(attribute, index) in form.attributes"
					:key="`space-${index}`"
					:class="{
						'border-b-2 pb-6': index !== form.attributes.length - 1,
					}"
				...
				>
					<div class="flex w-full justify-between gap-4 flex-wrap">
						...
						>
							<a-select
								v-model:value="form.attributes[index].attribute"
						...
						>
							<a-input
								v-model:value="
									form.attributes[index].measurement_unit
								"
						...
						>
							<a-select
								v-model:value="form.attributes[index].value"
...
export default {
	...
	data() {
		return {
			form: {
				name: null,
				attributes: [
					{ attribute: null, value: null, measurement_unit: null },
				],
			},
			rules: {
	...
	methods: {
		...
		setAttributeValue(value, option) {
			let attributes = this.attributesList.filter(
			...
			this.form.attributes[index].measurement_unit =
				attribute.measurement_unit ?? null;
		},
